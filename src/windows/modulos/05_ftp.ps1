...
# Se eliminó la llamada directa "Menu-FTP" del final del archivo
...
function Gestionar-InstalacionFTP {
    ...
    if (-not (Test-Path "$global:DIR_FTP_ROOT\LocalUser")) {
        New-Item -Path "$global:DIR_FTP_ROOT\LocalUser" -ItemType Directory -Force | Out-Null
    }

    if (-not (Test-Path "$global:DIR_FTP_MASTER\general")) {
        New-Item -Path "$global:DIR_FTP_MASTER\general" -ItemType Directory -Force | Out-Null
    }

    if (Test-Path "$global:DIR_FTP_ROOT\LocalUser\Public") {
        Remove-Item -Path "$global:DIR_FTP_ROOT\LocalUser\Public" -Force -Recurse -ErrorAction SilentlyContinue
    }
    New-Item -ItemType Junction -Path "$global:DIR_FTP_ROOT\LocalUser\Public" -Target "$global:DIR_FTP_MASTER\general" | Out-Null
    ...
    $AclRoot = Get-Acl $global:DIR_FTP_ROOT
    ...
    Set-Acl -Path $global:DIR_FTP_ROOT -AclObject $AclRoot

    $AclGeneral = Get-Acl "$global:DIR_FTP_MASTER\general"
    ...
    Set-Acl -Path "$global:DIR_FTP_MASTER\general" -AclObject $AclGeneral

    Write-Host "[*] Desplegando Sitio FTP..." -ForegroundColor Cyan
    $appcmd = "$env:systemroot\system32\inetsrv\appcmd.exe"
    
    Get-Website | Where-Object { $_.Bindings.Collection.BindingInformation -match ":21:" -and $_.Name -ne $global:SITE_NAME } | Stop-Website -ErrorAction SilentlyContinue
    cmd.exe /c "$appcmd delete site $global:SITE_NAME >nul 2>&1"
    
    & $appcmd add site /name:$global:SITE_NAME /bindings:ftp://*:21 /physicalPath:$global:DIR_FTP_ROOT
    & $appcmd set site $global:SITE_NAME "/ftpServer.security.ssl.controlChannelPolicy:SslAllow" "/ftpServer.security.ssl.dataChannelPolicy:SslAllow"
...
function Seleccionar-CrearGrupoFTP {
    $arr_grupos = @()
    if (Test-Path $global:DIR_FTP_MASTER) {
        Get-ChildItem -Path $global:DIR_FTP_MASTER -Directory | Where-Object { $_.Name -ne "general" } | ForEach-Object {
            $arr_grupos += $_.Name
        }
    }
    ...
    if ($eleccion -eq $arr_grupos.Count) {
        ...
        if (Confirmar-Accion "Crear/Utilizar el grupo '$nuevo_grupo' y su boveda compartida?") {
            ...
            $rutaGrupo = "$global:DIR_FTP_MASTER\$nuevo_grupo"
...
function Resetear-EntornoFTP {
    ...
    if (Confirmar-Accion "PELIGRO EXTREMO! Destruir Usuarios IIS, Grupos y Bovedas?") {
        $appcmd = "$env:systemroot\system32\inetsrv\appcmd.exe"
        & $appcmd delete site $global:SITE_NAME | Out-Null

        if (Test-Path "$global:DIR_FTP_ROOT\LocalUser") {
            Get-ChildItem -Path "$global:DIR_FTP_ROOT\LocalUser" -Directory | Where-Object { $_.Name -ne "Public" } | ForEach-Object {
        ...
        if (Test-Path $global:DIR_FTP_ROOT)   { Remove-Item -Path $global:DIR_FTP_ROOT   -Recurse -Force -ErrorAction SilentlyContinue }
        if (Test-Path $global:DIR_FTP_MASTER) { Remove-Item -Path $global:DIR_FTP_MASTER -Recurse -Force -ErrorAction SilentlyContinue }
...
function Menu-FTP {
    ...
    while ($true) {
        $estado   = if (Test-Path "$global:DIR_FTP_MASTER\general") { "ACTIVO" } else { "INACTIVO" }
