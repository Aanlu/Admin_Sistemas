...
function Procesar-UsuarioFTP {
    ...
    $usrFisico = "$DIR_FTP_ROOT\LocalUser\$Usuario"
    ...
    if (Test-Path $usrFisico) {
        ...
        foreach ($carpeta in $carpetas) {
            ...
            # Si hay una carpeta que no es del sistema (general/personal) ni es el nuevo grupo... ¡Es un grupo viejo!
            if ($nombreCarpeta -ne "general" -and $nombreCarpeta -ne "personal" -and $nombreCarpeta -ne $Grupo) {
                # 1. Quitamos el Junction para que visualmente DESAPAREZCA
                ...
                Write-Host "[-] El usuario '$Usuario' ha sido removido del grupo antiguo: '$nombreCarpeta'" -ForegroundColor DarkYellow
            }
        }
    }

    if (-not (Test-Path "$usrFisico\personal")) {
        New-Item -Path "$usrFisico\personal" -ItemType Directory -Force | Out-Null
    }
    ...
    if (-not (Test-Path $JunctionGeneral)) {
        New-Item -ItemType Junction -Path $JunctionGeneral -Target "$DIR_FTP_MASTER\general" | Out-Null
    }

    $JunctionGrupo = "$usrFisico\$Grupo"
    if (-not (Test-Path $JunctionGrupo)) {
        New-Item -ItemType Junction -Path $JunctionGrupo -Target "$DIR_FTP_MASTER\$Grupo" | Out-Null
    }
